...
import net.xnzn.app.selfdevice.utils.BitmapUtils
import net.xnzn.lib_log.L
import java.util.*
import kotlin.collections.ArrayList

class Camera2HelperFace(val mActivity: Activity, private val mTextureView: TextureView) {
    val TAG = "Camera2HelperFace"
...
//    private var mCameraSensorOrientation = 90                                            //摄像头方向
...
//        mCameraSensorOrientation = Surface.ROTATION_0
//        mCameraSensorOrientation = Surface.ROTATION_270

        Log.d(TAG, "initCameraInfo中mCameraSensorOrientation方向:: $mCameraSensorOrientation")
...
//            mTextureView.setAspectRatio(mPreviewSize.height, mPreviewSize.width)
        ...
        BitmapUtils.savePic(
            ...
            { savedPath, time ->
                mActivity.runOnUiThread {
                    Log.d(
                        TAG, "图片保存成功！ 保存路径：$savedPath 耗时：$time"
                    )
                }
            },
            { msg ->
    ...
    private fun initFaceDetect() {
        ...
        Log.d(
            TAG, "成像区域  ${activeArraySizeRect.width()}  ${activeArraySizeRect.height()} 比例: ${
        ...
        for (mode in faceDetectModes) {
            Log.d(TAG, "支持的人脸检测模式 $mode")
        }
        Log.d(TAG, "同时检测到人脸的数量 $faceDetectCount")
    }
    ...
    private fun handleFaces(result: TotalCaptureResult) {
        ...
        Log.d(TAG, "onCaptureCompleted  检测到 ${faces?.size} 张人脸")
    }
    ...
    private fun getBestSize(
        ...
        Log.d(
            TAG, "最大尺寸 ：$maxWidth * $maxHeight, 比例 ：${targetWidth.toFloat() / targetHeight}"
        )
        Log.d(
            TAG, "目标尺寸 ：$targetWidth * $targetHeight, 比例 ：${targetWidth.toFloat() / targetHeight}"
        )
    ...
    private fun exchangeWidthAndHeight(displayRotation: Int, sensorOrientation: Int): Boolean {
        ...
        when (displayRotation) {
            ...
            Surface.ROTATION_90, Surface.ROTATION_270 ->
                ...
                TAG, "Display rotation is invalid: $displayRotation"
            )
        }

        Log.d(
            TAG, "屏幕方向  $displayRotation"
        )
        Log.d(
            TAG, "相机方向  $sensorOrientation"
        )
        return exchange
    }
...
//        }
        ...
        Log.d(
            TAG, "configureTransform $viewWidth  $viewHeight"
        )
    }
