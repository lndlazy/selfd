package net.xnzn.app.selfdevice.qr;
        ...
        try {
            if (isYs()) {
                if (Build.MODEL.contains("3568")) {
                    mSerialPort = new SerialPort(new File("/dev/ttyS4"), 9600);
                } else {
...
//            outputStream.write(new byte[]{0x40,0x05,0x24,0x00,0x00,0x00,0x00,0x00});
//            outputStream.flush();
